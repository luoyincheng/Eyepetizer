...
public class MagicScene extends RunOnDraw {
    ...
    public float getWidth() {
        Log.i("glsurface", "mSceneSize.width():" + mSceneSize.width());
    ...
    private boolean prepare() {
        ...
        mIsPreparing = true;
        try {
            Log.i("cycleWay", "MagicScene:prepare():mSurfaces.prepare()");
            for (MagicBaseSurface<?> s : mSurfaces) {
                s.prepare();
            }
            Log.i("cycleWay", "MagicScene:updateCamera() & MagicScene:updateFrustum()");
            updateCamera();
            updateFrustum();
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    void init(String vertexShader, String fragmentShader) {
        ...
        mInited = false;
        Log.i("cycleWay", "MagicScene:init():addRunOnDraw()");
    ...
    private void init() {
        if (!mInited && isPrepared()) {
            ...
            for (int i = 0; i < mSurfaces.size(); ++i) {
                ...
                textureIndex++;
                Log.i("cycleWay", "MagicScene.init():" + s.mTextures.size());
    ...
    Rect getSceneViewRect() {
        if (mSceneViewRect == null) {
            ...
            Log.i("cycleWay", "MagicScene:getSceneViewRect():" + mSceneViewRect.toString());
        }
    ...
    private int getTextureCount() {
        int c = 0;
        Log.i("cycleWay", "MagicScene.getTextureCount()-pre:" + mSurfaces.size());
        for (MagicBaseSurface<?> s : mSurfaces) {
            c += (1 + s.mTextures.size());
        }
        Log.i("cycleWay", "MagicScene.getTextureCount()-after:" + mSurfaces.size());
        Log.i("cycleWay", "前后数据一样说明textures数据为0");
        return c;
    }
