package com.yincheng.eyepetizer.widgets.doublesideview;
...
import com.yincheng.eyepetizer.widgets.helper.ViewDragHelper;
...
/**
 * DoubleSideView acts as a top-level container for window content that allows for
 ...
 * </p>
 *
 * <p>To use a DoubleSideView, position your primary content view as the first child with
 ...
 * and elsewhere.</p>
 *
 * <p>For more information about how to use DoubleSideView, read <a
 * href="{@docRoot}training/implementing-navigation/nav-drawer.html">Creating a Navigation
 * Drawer</a>.</p>
 * 1.双向的drawerLayout,Drawer有两种表现形式，覆盖或者不覆盖内容区域
 * 2.滑动带有阻尼效果，在滑动速度小于预设的值时候，无论滑动多少距离，Drawer都不会显示。适用与idle状态下，无论是Drawer完全显示还是完全隐藏。
 * 3.只有在滑动速度和滑动距离都达到预设值的时候才会触发显示或者隐藏动画
 * 4.在阈值内会随着手指的滑动距离的增大慢慢减小漏出的速度，比如刚开始手指在contentView中向右边滑动20px，则左边的Drawer也漏出来20px，
 * 但是越往后面，手指滑动20px可能Drawer只会显示2px
 * 5.当左侧漏出的像素值达到预设的某一个值时，drawer会立即执行显示动画显示出来，这个时候可以设置加速器，无论怎样，这个过程中都会忽略用户的输入事件，
 * 直到整个DoubleSideView完全settle(idle)
 * 6.在Drawer展开状态下也是同样的效果
 * 7.
 */

public class CompoundDrawerLayout extends ViewGroup {
    ...
    private static final String TAG = "DoubleSideView";
    ...
    private float mMinimumFlingVelocity, mMaximumFlingVelocity;
    ...
    public CompoundDrawerLayout(@NonNull Context context) {
        this(context, null);
    }

    public CompoundDrawerLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CompoundDrawerLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyle) {
        ...
        mMinimumFlingVelocity = configuration.getScaledMinimumFlingVelocity() * 10;
         ...
         * 在Drawer展开状态下按了返回键，会关闭Drawer而不是。。。
         */
...
//        setFocusable(false);
            ...
            if (Build.VERSION.SDK_INT >= 21) {
                setOnApplyWindowInsetsListener(new View.OnApplyWindowInsetsListener() {
                    ...
                    public WindowInsets onApplyWindowInsets(View view, WindowInsets insets) {
                        final CompoundDrawerLayout compoundDrawerLayout = (CompoundDrawerLayout) view;
                        compoundDrawerLayout.setChildInsets(insets, insets.getSystemWindowInsetTop() > 0);
     ...
     * any drawer within this layout. DoubleSideView will still respond to calls to
     ...
     *
     * <p>This allows the application to restrict the user's ability to open or close
     * the given drawer. DoubleSideView will still respond to calls to {@link #openDrawer(int)},
    ...
    /**
     * Returns true if x and y coord in DoubleSideView's coordinate space are inside the bounds of the
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        Log.i("fasfadfadfafafd", Gravity.LEFT + ":" + Gravity.START);
        ...
        if (widthMode != MeasureSpec.EXACTLY || heightMode != MeasureSpec.EXACTLY) {
            if (isInEditMode()) {
                ...
                throw new IllegalArgumentException(
                        "DoubleSideView must be measured with MeasureSpec.EXACTLY.");
            }
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (isContentView(child)) {
                Log.i("layoutyixia", "content");
                int contentOffSet = 0;
                for (int j = 0; j < getChildCount(); j++) {
                    final View view = getChildAt(j);
                    ViewGroup.LayoutParams layoutParams = view.getLayoutParams();
                    if (layoutParams instanceof LayoutParams && ((LayoutParams) layoutParams).onScreen != 0.0f) {
                        contentOffSet = (int) (view.getMeasuredWidth() * ((LayoutParams) layoutParams).onScreen);
                        contentOffSet = ((LayoutParams) layoutParams).gravity == Gravity.START ? contentOffSet : -contentOffSet;
                        break;
                    }
                }
                child.layout(lp.leftMargin + contentOffSet, lp.topMargin,
                        lp.leftMargin + contentOffSet + child.getMeasuredWidth(),
                        lp.topMargin + child.getMeasuredHeight());
            } else { // Drawer, if it wasn't onMeasure would have thrown an exception.
                Log.i("layoutyixia", "not content");
                Log.i("onLayoutLayout", lp.leftMargin + ":" + (lp.leftMargin + child.getMeasuredWidth()));
     ...
     * Note that this will only be activated if this DoubleSideView fitsSystemWindows.
     *
...
//        c.drawRect(rect, paint);
    ...
    private boolean invalidateContentAndDrawer(Canvas canvas, View child, long drawingTime) {
        if (isContentView(child)) return false;
        final int restoreCount = canvas.save();
        View contentView = null;
        for (int i = 0; i < getChildCount(); i++) {
            final View view = getChildAt(i);
            if (isContentView(view)) contentView = view;
        }
        if (contentView == null) return false;
        if (child.getLeft() < contentView.getLeft()) {
            contentView.setLeft(child.getRight());
            contentView.setRight(contentView.getLeft() + contentView.getWidth());
        } else {
//            contentView.setRight(child.getLeft());
//            contentView.setLeft(contentView.getRight() - contentView.getWidth());

            contentView.setLeft(child.getRight());
            contentView.setRight(contentView.getLeft() + contentView.getWidth());
        }
        boolean isInvalidateChild = super.drawChild(canvas, child, drawingTime);
        boolean isInvalidateContent = super.drawChild(canvas, contentView, drawingTime);
        canvas.restoreToCount(restoreCount);
        return (isInvalidateChild && isInvalidateContent);
    }

    @Override
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        Log.i("drawingTimes", "drawChild" + child);
        return super.drawChild(canvas, child, drawingTime);
//        return invalidateContentAndDrawer(canvas, child, drawingTime);
//        final int height = getHeight();
//        final boolean drawingContent = isContentView(child);
//        Log.i("viewDragHelper", "drawingContent：" + drawingContent +":"+ child.getLeft());
//        int clipLeft = 0, clipRight = getWidth();
//
//        final int restoreCount = canvas.save();
//        if (drawingContent) {
//            Log.i("viewDragHelper", "绘制content");
//            final int childCount = getChildCount();
//            for (int i = 0; i < childCount; i++) {
...
//        return result;
    ...
    /*
     * 根据是否设置了gravity来判断是不是ContentView
     */
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        Log.i("findTopChildUnder", "interceptForTap" + interceptForTap);
        Log.i("findTopChildUnder", "onInterceptTouchEvent" + (interceptForDrag || interceptForTap || hasPeekingDrawer() || mChildrenCanceledTouch));
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        Log.i("onTouchEvent", action + ":" + MotionEvent.ACTION_MASK);
        Log.i("onTouchEvent", "-->" + (action & MotionEvent.ACTION_MASK));
        // TODO: 19-8-14  ，为什么过滤掉高于255的action
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if ((Math.abs(velocityX) > mMinimumFlingVelocity)) {
                    Log.i("flingTest", "=============>" + mMaximumFlingVelocity);
                }
                break;
            ...
            case MotionEvent.ACTION_UP: {
                ...
                final int pointerId1 = ev.getPointerId(0);
                velocityTracker1.computeCurrentVelocity(1000, mMaximumFlingVelocity);
                final float velocityX1 = velocityTracker1.getXVelocity(pointerId1);
                if ((Math.abs(velocityX1) > mMinimumFlingVelocity)) {
                    Log.i("flingTest", "-------------->" + velocityX1);
                }
    ...
    private class ViewDragCallback extends ViewDragHelper.Callback {
        ...
        // TODO: 19-8-14  ,这个方法在findTopChildUnder()实现中会用到
//        @Override
...
//        }
        ...
        public void removeCallbacks() {
            CompoundDrawerLayout.this.removeCallbacks(mPeekRunnable);
        }
        ...
        public void onViewPositionChanged(@NonNull View changedView, int left, int top, int dx, int dy) {
            Log.i("viewDragHelper", "onViewPositionChanged" + changedView + ":" + changedView.getLeft());
            float offset;
        ...
        @Override
        public Pair<View, View> onViewCaptured(@NonNull View capturedChild, int activePointerId) {
            final LayoutParams lp = (LayoutParams) capturedChild.getLayoutParams();
            Log.i("viewDragHelper", "onViewCaptured:" + lp.gravity + ":" + Gravity.START + ":" + Gravity.END);
            lp.isPeeking = false;
            closeOtherDrawer();
            View contentView = null;
            for (int i = 0; i < getChildCount(); i++) {
            ...
            if (Gravity.START == lp.gravity) return new Pair<>(null, contentView);
            if (Gravity.END == lp.gravity) return new Pair<>(contentView, null);
            return null;
        }

        private void closeOtherDrawer() {
            Log.i("viewDragHelper", "closeOtherDrawer");
        ...
        public void onEdgeTouched(int edgeFlags, int pointerId) {
            Log.i("viewDragHelper", "onEdgeTouched------------------" + System.currentTimeMillis());
            postDelayed(mPeekRunnable, PEEK_DELAY);
        }

        void peekDrawer() {
            Log.i("viewDragHelper", "peekDrawer-------------------" + System.currentTimeMillis());
        ...
        public int clampViewPositionHorizontal(View child, int left, int dx) {
            Log.i("viewDragHelper", "clampViewPositionHorizontal");
            Log.i("drawingTimes", "clampViewPositionHorizontal");
            int result = 0;
            if (checkDrawerViewAbsoluteGravity(child, Gravity.LEFT)) {
                result = Math.max(-child.getWidth(), Math.min(left, 0));
            } else {
                final int width = getWidth();
                result = Math.max(width - child.getWidth(), Math.min(left, width));
            }

            Log.i("horizontalposition", "result:" + result);
            return result;
        }
    }
    ...
    class AccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public void onInitializeAccessibilityNodeInfo(View host, AccessibilityNodeInfoCompat info) {
            ...
            info.setClassName(CompoundDrawerLayout.class.getName());
        ...
        public void onInitializeAccessibilityEvent(View host, AccessibilityEvent event) {
            ...
            event.setClassName(CompoundDrawerLayout.class.getName());
        }
        ...
        public boolean dispatchPopulateAccessibilityEvent(View host, AccessibilityEvent event) {
            ...
            // DoubleSideView invalidate the entire contents of the screen (like
