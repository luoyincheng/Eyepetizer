...
public abstract class MagicBaseSurface<T> {
    ...
    void setProgram(Program program) {
        ...
        Log.i("cycleWay", "MagicBaseSurface.mTextures.size():" + mTextures.size());
    ...
    void prepare() {
        ...
        if (!mIsView) {
            ...
            Executors.newSingleThreadExecutor().submit(new Runnable() {
                @Override
                public void run() {
                    Log.i("cycleWay", "获取要进行动画展示的那个View:" + mView.getWidth() + ":" + mView.getHeight());
                    mBmpBody = ViewUtil.getDrawingCache(mView);
                    Log.i("cycleWay", "将该View转化为Bitmap:并将Bitmap赋值给texture" + mBmpBody.getWidth() + ":" + mBmpBody.getHeight());
                    mBody.value().mBmp = mBmpBody;
                    mIsPrepared = true;
                    mIsPreparing = false;
                }
    ...
    private void initTextures(Bitmap... bitmaps) {
        ...
        Log.i("cycleWay", "MagicBaseSurface:initTextures():" + bitmaps.length);
